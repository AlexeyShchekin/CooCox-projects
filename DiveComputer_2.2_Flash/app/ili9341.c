...
void TFT_DrawString ( uint16_t x, uint16_t y, char* s, int len,
...
void TFT_DMA_DrawString ( uint16_t x, uint16_t y, char* s, int len,
		uint16_t color, uint16_t backColor, uint8_t size)
{
	int i;//, len = strlen(s);
...
void TFT_DMA_DrawChar10x16(uint16_t X, uint16_t Y, uint8_t chr, uint16_t color, uint16_t backcolor, uint8_t size)
{
	uint16_t i,j,n,m;
	uint16_t buffer[10];
	...
	TFT_setCol ( X, X + 10*size - 1 );
	TFT_setPage ( Y, Y+16*size);
	TFT_sendCMD ( 0x2c );
...
void TFT_DMA_DrawStr10x16(uint8_t X, uint8_t Y, char *str, uint16_t length, uint16_t color, uint16_t backcolor, uint8_t size)
{
	int i;
	for (i=0; i<length; i++)
	{
		TFT_DMA_DrawChar10x16(X,Y,str[i],color,backcolor, size);
		if (X < 320 - 10*size) { X += 10*size; } else if (Y < 240 - 16*size) { X = 0; Y += 16*size; } else { X = 0; Y = 0; }
	}
   /* while (*str) {
