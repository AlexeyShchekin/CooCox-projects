...
void ST7735_SPI_SpeedSwitch(void)
{
	SPI_InitTypeDef SPI;
...
void ST7735_Init(void) {
	...
	PORT.GPIO_Pin = GPIO_Pin_15;//|GPIO_Pin_9
	PORT.GPIO_Speed = GPIO_Speed_50MHz;
	PORT.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init(GPIOB,&PORT);

	GPIOB->BSRR |= GPIO_BSRR_BS15;// | GPIO_BSRR_BS9;
...
void ST7735_PutStr5x7_DMA(uint8_t X, uint8_t Y, char *str, uint16_t length, uint16_t color, uint16_t backcolor, uint8_t size)
{
	int i;
	for (i=0; i<length; i++)
	{
		ST7735_PutChar5x7_DMA(X,Y,str[i],color,backcolor,size);
...
void ST7735_PutStr7x11_DMA(uint8_t X, uint8_t Y, char *str, uint16_t length, uint16_t color, uint16_t backcolor,uint8_t size)
{
	int i;
	for (i=0; i<length; i++)
	{
		ST7735_PutChar7x11_DMA(X,Y,str[i],color,backcolor,size);
...
void ST7735_PutStr10x16_DMA(uint8_t X, uint8_t Y, char *str, uint16_t length, uint16_t color, uint16_t backcolor, uint8_t size)
{
	int i;
	for (i=0; i<length; i++)
	{
		ST7735_PutChar10x16_DMA(X,Y,str[i],color,backcolor, size);
