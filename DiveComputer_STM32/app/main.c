...
void GetSensorMeasurements(void)
{
	...
	readData(C,Data);
	Sensor_OFF();
	CurrentData[0] = ((double)Data[0])/10;
	CurrentData[1] = (((double)Data[1])-SensCalibr)/101.325;
}
...
int main(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_2 | GPIO_Pin_3;		// INPUT BUTTONS
    ...
    while(1)
    {
		...
		else //if (STATE==RUN_STATE)
		{
			...
			else if ((STATE&0x3000)!=0x3000)//PWR_FLAG==0)
			{
				...
				{
					init_clock();
					if (TFT_FLAG==0x00) {GPIOA->BSRR = GPIO_BSRR_BS14;}
					T = RTC_GetCounter();
					if (T>=0x01517FFF)
					{
						DT = GetDate();
			...
			else if ((STATE&0x3000)==0x3000)//(PWR_FLAG == 1)
			{
				...
				init_clock();
				Delay_ms(400);
				GPIOA->BSRR = GPIO_BSRR_BR14 | GPIO_BSRR_BR15;
